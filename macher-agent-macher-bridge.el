  ...
  (let ((virtual-edits nil)
    ...
    (dolist (entry contents)
      (let* ((name (car entry))
             ...
             (abs-path (expand-file-name name root-dir))
        ...
        (if is-pure-virtual
            (let ((old-text (with-current-buffer live-buf
              ...
              (push (cons name (cons old-text new-text)) virtual-edits))
          ;; Route to physical patch and strictly read from disk
          (let ((old-text (when (file-exists-p abs-path)
                            (with-temp-buffer
                              (insert-file-contents abs-path)
                              (buffer-substring-no-properties (point-min) (point-max))))))
            (push (cons name (cons old-text new-text)) physical-edits)))))
...
(defun macher-agent--generate-gnu-diff (context &rest _args)
  ...
  (let* ((contents (macher-context-contents context))
         (raw-ws (macher-agent--get-context-workspace context))
         (root-dir (if (fboundp 'macher--workspace-root)
                       (ignore-errors (macher--workspace-root raw-ws))
                     default-directory))
         (edits nil))
    (dolist (entry contents)
      (let* ((rel-path (car entry))
             (new-text (cdr entry))
             (abs-path (expand-file-name rel-path root-dir))
             (old-text (when (file-exists-p abs-path)
                         (with-temp-buffer
                           (insert-file-contents abs-path)
                           (buffer-substring-no-properties (point-min) (point-max))))))
...
(defun macher-agent--build-patch (context &optional _fsm)
  ...
  (let* ((raw-workspace (if (fboundp 'macher-agent--get-context-workspace)
         ...
         (root-dir (if (fboundp 'macher--workspace-root) (ignore-errors (macher--workspace-root valid-workspace)) default-directory))
         (proj-name (if (fboundp 'macher--workspace-name) (ignore-errors (macher--workspace-name valid-workspace)) "unknown"))
